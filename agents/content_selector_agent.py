...
from .job_analyzer_agent import JobAnalysis # Import the input schema from Phase 2

# A sub-model for a single, rewritten work experience entry
class TailoredWorkExperience(BaseModel):
    ...
    # This is the most important field - the rewritten bullet points
...
# A sub-model for a single, rewritten project entry
...
# The main Pydantic model for the entire tailored resume content
class TailoredResumeContent(BaseModel):
    """The final, tailored content for the new resume."""
    ...
    selected_projects: List[TailoredProject] = Field(description="The 1-2 most relevant projects, with descriptions tailored to the job.")
...
def select_and_tailor_content(job_analysis: JobAnalysis, master_profile: Dict) -> TailoredResumeContent:
    """
    Takes a job analysis and a master profile, and returns tailored resume content.

    Args:
        job_analysis (JobAnalysis): The structured analysis of the target job.
        master_profile (Dict): The user's complete professional history.

    Returns:
        TailoredResumeContent: A Pydantic object with the selected and rewritten content.
    """
    # Use a more powerful model like GPT-4 for this reasoning-heavy task.
    llm = ChatOpenAI(model="gpt-4o", temperature=0.2)
    parser = PydanticOutputParser(pydantic_object=TailoredResumeContent)

    # Convert the Pydantic object and dictionary to JSON strings to pass into the prompt
    ...
    **Your Instructions:**
    ...
    2.  **Work Experience:** Select the 2 most relevant work experiences from the master profile that best match the job analysis. For EACH selected experience:
        -   DO NOT change the company, role, or dates.
        -   **REWRITE the bullet points.** Infuse them with keywords from the job analysis's `key_skills` and `core_responsibilities`. Rephrase accomplishments to directly address what the job is looking for. Focus on quantifiable results where possible.
        
    3.  **Projects:** Select the a few relevant projects. For EACH selected project:
        -   **REWRITE the description.** Make it concise and highlight the technologies and outcomes that are relevant to the target job.
        
    4.  **Skills:** From the master profile's skills, create a new list of skills that are most relevant to the `key_skills` required by the job. Maintain the original categories (e.g., 'programming_languages', 'technologies').
